...
const emit = defineEmits<{
  ...
  (e: 'create'): void
  (e: 'edit', educationCenterId: number): void
  (e: 'delete', educationCenterId: number): void
}>()

const headers = [
  { title: 'ID', key: 'educationCenterId' },
...
<template>
  <VCard>
    ...
    >
      <template #item.educationCenterId="{ item }">
        <span class="text-body-2 font-weight-medium">{{ item?.educationCenterId ?? '-' }}</span>
      </template>
      ...
      <template #item.actions="{ item }">
        <div class="d-flex gap-2">
          <VBtn
            ...
            size="small"
            :disabled="!item"
            @click="item && emit('edit', item.educationCenterId)"
          >
          ...
          <VBtn
            ...
            color="error"
            :disabled="!item"
            @click="item && emit('delete', item.educationCenterId)"
          >
