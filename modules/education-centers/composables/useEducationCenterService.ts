...
export interface EducationCenterLegacy {
...
const educationCenters = ref<EducationCenterLegacy[]>([])
const totalEducationCenters = ref(0)
const loadingList = ref(false)
const loadingForm = ref(false)
const loadingSave = ref(false)

function mapBackendEducationCenter(e: any): EducationCenterLegacy {
  return {
...
async function fetchEducationCenters(page: number, size: number) {
  ...
  try {
    const response = await useApi<ApiResponse<EducationCenterLegacy>>(
...
async function getEducationCenter(id: number) {
  ...
  try {
    const response = await useApi<{ data: EducationCenterLegacy }>(
...
async function updateEducationCenter(id: number, center: any) {
  loadingSave.value = true
  try {
    await useApi<{ data: EducationCenterLegacy }>(
      `/graduate-insights/v1/api/education_center/${id}`,
      {
        method: 'PUT',
        body: center,
