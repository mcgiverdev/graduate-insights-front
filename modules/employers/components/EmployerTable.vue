...
const emit = defineEmits<{
  ...
  (e: 'create'): void
  (e: 'edit', employerId: number): void
  (e: 'delete', employerId: number): void
}>()
...
<template>
  <VCard>
    ...
    >
      <template #item.employerId="{ item }">
        <span class="text-body-2 font-weight-medium">{{ item.employerId ?? '-' }}</span>
      </template>
      ...
      <template #item.actions="{ item }">
        <div class="d-flex gap-2">
          <VBtn
            ...
            size="small"
            :disabled="!item"
            @click="item && emit('edit', item.employerId)"
          >
          ...
          <VBtn
            ...
            color="error"
            :disabled="!item"
            @click="item && emit('delete', item.employerId)"
          >
