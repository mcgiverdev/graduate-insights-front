...
const emit = defineEmits<{
  ...
  (e: 'create'): void
  (e: 'edit', directorId: number): void
  (e: 'delete', directorId: number): void
}>()

const headers = [
  { title: 'ID', key: 'directorId' },
  { title: 'DNI', key: 'dni' },
...
<template>
  <VCard>
    ...
    >
      <template #item.directorId="{ item }">
        <span class="text-body-2 font-weight-medium">{{ item?.directorId ?? '-' }}</span>
      </template>
      ...
      <template #item.actions="{ item }">
        <div class="d-flex gap-2">
          <VBtn
            ...
            size="small"
            :disabled="!item"
            @click="item && emit('edit', item.directorId)"
          >
          ...
          <VBtn
            ...
            color="error"
            :disabled="!item"
            @click="item && emit('delete', item.directorId)"
          >
