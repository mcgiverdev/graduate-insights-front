...
function mapBackendDirector(d: any): Director {
  return {
...
async function fetchDirectors(page: number, size: number) {
  ...
  try {
    const response = await useApi<ApiResponse<Director>>(
...
async function getDirector(id: number) {
  ...
  try {
    const response = await useApi<{ data: Director }>(
